    ...
    <div v-if="session.user_id != null" class="send-page">
      <div class="container">
        <form @submit.prevent="submit" class="send-form">
          <label for="send_content" class="sr-only">Thread Content</label>
          <textarea v-model="send.content" type="text" id="send_content" class="form-control" rows="3" placeholder="Content" required autofocus></textarea>
...
<script>
  ...
  export default {
    ...
    methods: {
      submit () {
        const self = this
        const send_to = this.$route.params.user_id
        const send = {
          to : send_to,
          content : self.send.content
        }
        $.post(api + '/send?user_id=' + send_to, send).done((data) => {
          if (data.toString() === '1') {
